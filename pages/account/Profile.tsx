...
export default function Profile() {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      <h2 className="text-2xl font-semibold tracking-tight">{t('profile', { ns: 'account' })}</h2>
      ...
      <div className="bg-canvas border border-border/50 rounded-[2rem] p-6 sm:p-10 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {success && <div className="p-4 bg-green-50 text-green-600 rounded-xl text-sm">{t('profile_updated', { ns: 'account' })}</div>}
          ...
          <div>
            <label htmlFor="email" className="block text-sm font-medium mb-2 text-fg-muted">{t('email_address', { ns: 'account' })}</label>
            <input 
              id="email"
              type="email" 
              value={user?.email || ''}
            ...
            />
            <p className="text-xs text-fg-muted mt-2">{t('email_cannot_change', { ns: 'account' })}</p>
          </div>

          <div>
            <label htmlFor="phone" className="block text-sm font-medium mb-2 text-fg-muted">{t('phone', { ns: 'checkout' })}</label>
            <input 
              id="phone"
              type="tel" 
              value={phone}
